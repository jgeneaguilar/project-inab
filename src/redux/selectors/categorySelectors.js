...
export const getCategoriesByMasterCategory = createSelector(
  ...
  (masterCategories, categories, categoryBudgets, transactions) => {
    const _categories = groupArrayToId(categories, 'master_category_id', (category) => {
      const budgeted = categoryBudgets[category._id]
        ? categoryBudgets[category._id].budgeted / 100
        : 0;
      const activity = transactions
        .filter((item) => item.category_id === category._id)
        .reduce((total, item) => (total = total + item.amount / 100), 0);
      return {
        key: category._id,
        type: 'category',
    ...
    return masterCategories.map((item) => {
      ...
      return {
        ...
        category: item.name,
        budgeted: _cat
          ? _cat.reduce((total, item) => (total = total + item.rawBudgeted), 0).toFixed(2)
          : 0,
        activity: _cat
          ? _cat.reduce((total, item) => (total = total + item.rawActivity), 0).toFixed(2)
          : 0,
        available: _cat
          ? _cat
              .reduce((total, item) => (total = total + item.rawAvailable), 0)
              .toFixed(2)
          : 0,
        children: _cat || [],
      };
